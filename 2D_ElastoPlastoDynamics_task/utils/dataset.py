...
class VKIFlowDataset(Dataset):
    ...
    def __init__(
        ...
        self.VKILS_dataset = VKILS_dataset
        self._raw_geom_latents = np.array(geom_latents)
        total = len(VKILS_dataset)
    ...
    def compute_norm_params(self):
        ...
        for idx in range(len(self.VKILS_dataset)):
            item = list(self.VKILS_dataset.values())[idx]
            ...
            if self.mode in ('train','val'):
                ...
                mach_all.append(np.array(item['mach']).flatten())
                nut_all.append(np.array(item['nut']).flatten())




            angles = [float(item['angle_in']), float(item['mach_out'])]
    ...
    def process_dataset(self):
        data_list = []
        c = self.coef_norm
        skeys = ['Q', 'power', 'Pr', 'Tr', 'eth_is', 'angle_out']

        for idx in range(len(self.VKILS_dataset)):

            item = list(self.VKILS_dataset.values())[idx]
            pts = np.array(item.pos)
            sdf = np.array(item.x[:,2])   #.[:,2] pour selectionner la sdf
            ...
            if self.mode in ('train','val'):
                ...
                mach = np.array(item['mach']).flatten()
                nut  = np.array(item['nut']).flatten()
    ...
    def split_val(self, val_size, seed=None):
        ...
        train_ds = VKIFlowDataset(self.VKILS_dataset, self._raw_geom_latents,
                                  mode='train', num_points=self.num_points,
                                  idx_list=train_idx)
        val_ds   = VKIFlowDataset(self.VKILS_dataset, self._raw_geom_latents,
...
class vkiSDFDataset(Dataset):
    ...
    """
    def __init__(self, VKILS_dataset, is_train=True, coef_norm=None, num_points=None,
