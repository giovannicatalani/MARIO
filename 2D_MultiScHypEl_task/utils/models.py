...
try:
    from utils.conditioning import CrossAttentionModulation
except:
    from conditioning import CrossAttentionModulation



try:
    from utils.conditioning import cross_attention_conditioning
except:
    from conditioning import cross_attention_conditioning
...
class MultiScaleModulatedFourierFeatures(nn.Module):
    def __init__(
        ...
        if self.conditioning_type == 'shift_modulation':
            ...
            self.conditioning = cross_attention_conditioning
        ...
        self.cross_attention_layer_multihead = CrossAttentionModulation(self.hidden_dim, 0, self.hidden_dim, embedding_size=16, n_heads=2, dropout=0.1)
    ...
    def modulated_forward(self, x, z):
        ...
        if self.conditioning_type == 'attention' :
            ...
            pre_outs = [self.conditioning(pos, features, self.layers[:-1],  torch.relu, self.cross_attention_layer_multihead, with_batch=True) for pos in positions]
