...
def main(cfg: DictConfig) -> None:
    ...
    train_ds = Hyper_Elastic_2D_FlowDataset(
        Hyper_elas   = train_raw,
    ...
    test_dataset = Hyper_Elastic_2D_FlowDataset(
        Hyper_elas   = test_raw,
    ...
    for step in tqdm(range(start_epoch, epochs)):
        ...
        if use_pred_loss:
            ...
            plt.ylabel('Loss')
            plt.yscale('log')
            plt.title(f'Training and Test Loss ,conditionning {cfg.inr.conditionning_type}')
            plt.legend()
    ...
    for i in range (len(results['fields_pred'])):
        ...
        for j, fn in enumerate(['u1', 'u2', 'P11', 'P12', 'P22', 'P21', 'psi']):
            reference[-1][fn] = results['fields_pred'][i][:,j]
        for sn in out_scalars_names:
            reference[-1][sn] = np.array(results['scalars_pred'][i][0])
