...
class MOE_network(nn.Module):
    def __init__(self,input_dim, num_experts , latent_dim):
        super().__init__()
        self.fc1 = nn.Linear(input_dim,latent_dim)
        self.fc2 = nn.Linear(latent_dim,latent_dim)
        self.fc3 = nn.Linear(latent_dim, num_experts)

    def forward(self,x):
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        alpha_expert = F.softmax(self.fc3(x) , dim = -1)
...
class MultiScaleModulatedFourierFeatures(nn.Module):
    def __init__(
        ...
        self.MOE_expert = MOE_network(input_dim = input_dim, num_experts = len(self.scales) , latent_dim =  self.latent_dim)    #############
    ...
    def modulated_forward(self, x, z):
        ...
        inter = alpha_MOE * stacked_expert
