...
from utils.dataset import Tensil_2D_flowDataset, subsample_dataset
...
def main(cfg: DictConfig) -> None:
    ...
    if device != 'cuda' :
        assert('Attention : Utilisation du modèle du CPU !')

    else :
        print("Utilisation du modèle sur GPU ! ")
    ...
    train_ds = Tensil_2D_flowDataset(
        Tensil_2d   = train_raw,
    ...
    test_dataset = Tensil_2D_flowDataset(
        Tensil_2d   = test_raw,
    ...
    out_scalar_name = ['max_von_mises',  'max_U2_top', 'max_sig22_top']

    reference = []
    for i in range (len(results['fields_pred'])):
        reference.append({})
        for j, fn in enumerate(out_fields_names):
            if fn == 'q':
                pass
            else :
                reference[-1][fn] = results['fields_pred'][i][:,j]
        for k,sn in enumerate(out_scalar_name):
            if sn == 'max_q':
                pass
            else :
                reference[-1][sn] = np.array(results['scalars_pred'][i][k])
